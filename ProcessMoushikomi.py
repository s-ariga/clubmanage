...
@s_ariga seiichi-ariga@gmail.com 2024
...
if __name__ == "__main__":
    # Excelファイルを探す
    # この時点で、../data/の中のxlsxファイルをExcelで開いていると、
    # "~"のつく一時ファイルも、対象になるので、全部閉じること
    ...
    for file in datalist:
        ...
        if team_name == "":
            print("team_name が空白です")
            print(file)
            exit()
        # チームのデータを登録リストに追加
        shashu_list = pd.concat(
            [shashu_list, shashu_data], sort=False, ignore_index=True
        )
        ...
        sankahi_list = pd.concat(
            [cf.sankahi_calc(shashu=shashu_data, team=team_data), sankahi_list],
            sort=False,
            ignore_index=True,
        )
    # 10m競技のリスト読み込み
